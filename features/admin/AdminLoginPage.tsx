...
// ไอคอนยังคงเดิม ไม่มีการเปลี่ยนแปลง
        ...
        <Card className="shadow-2xl border border-blue-100">
          <CardContent className="p-10 sm:p-12">
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="text-center text-3xl font-extrabold text-gray-900 tracking-tight">{t('adminLoginPage.title')}</h2>
              <p className="text-gray-500 text-sm mt-2">{t('adminLoginPage.subtitle')}</p>
            </div>
            <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
              {error && <ErrorMessage message={error} onDismiss={() => setError(null)} title={t('adminLoginPage.errorTitle')} />}
              <InputField
                label={t('adminLoginPage.emailOrUsernameLabel')}
              ...
              />
              <InputField
                label={t('adminLoginPage.passwordLabel')}
                id="password"
                name="password"
                type="password"
