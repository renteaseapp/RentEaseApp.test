...
import { getRentalByIdOrUid, approveRentalRequest, rejectRentalRequest, markPaymentSlipInvalid, processReturn, verifySlipByImage, verifyRentalPayment } from '../../services/rentalService';
...
export const OwnerRentalDetailPage: React.FC = () => {
  ...
  const showDeliveryUpdateForm = rental.pickup_method === 'delivery' && ['confirmed', 'active', 'shipped'].includes(rental.rental_status);
                ...
                <div className="md:col-span-2">
                  ...
                  />
                    ...
                    <div className="md:col-span-3 space-y-4">
                      <h3 className="text-2xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors">
                        <Link to={ROUTE_PATHS.PRODUCT_DETAIL.replace(':productId', String(rental.product_id))} className="hover:underline">
                  ...
                  <DetailItem 
                    ...
                    label={t('ownerRentalDetailPage.labels.pickupMethod')}
                        value={<span className="capitalize">{t(`ownerRentalDetailPage.pickupMethod.${rental.pickup_method.toLowerCase()}`)}</span>}
                ...
                />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      required
                    >
                          <option value="pending">{t('ownerRentalDetailPage.status.delivery.pending')}</option>
                          <option value="shipped">{t('ownerRentalDetailPage.status.delivery.shipped')}</option>
                          <option value="delivered">{t('ownerRentalDetailPage.status.delivery.delivered')}</option>
                          <option value="failed">{t('ownerRentalDetailPage.status.delivery.failed')}</option>
                          <option value="returned">{t('ownerRentalDetailPage.status.delivery.returned')}</option>
                    </select>
                  </div>
                  <div>
    ...
    <Modal onClose={onClose}>
      <h2 className="text-xl font-bold text-gray-900 mb-4">{t('ownerRentalDetailPage.returnForm.title')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
          <label htmlFor="returnImages" className="block font-medium text-gray-700 mb-1">{t('ownerRentalDetailPage.returnForm.images')}</label>
          <input 
            id="returnImages"
            type="file" 
            multiple 
