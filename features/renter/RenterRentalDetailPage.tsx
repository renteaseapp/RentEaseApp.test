...
export const RenterRentalDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    if (realtimeRental && rental) {
      ...
      if (rental.rental_status !== realtimeRental.rental_status) {
        ...
        if (message) {
          setRealtimeNotification({ isVisible: true, message, type: 'success' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        }
        if (realtimeRental.rental_status === 'late_return') {
          setRealtimeNotification({ isVisible: true, message: t('renterRentalDetailPage.alerts.lateReturnDetected'), type: 'warning' });
      ...
      if (rental.payment_status !== realtimeRental.payment_status) {
        if (realtimeRental.payment_status === 'paid') {
          setRealtimeNotification({ isVisible: true, message: t('renterRentalDetailPage.alerts.paymentConfirmed'), type: 'success' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        } else if (realtimeRental.payment_status === 'pending_verification') {
          setRealtimeNotification({ isVisible: true, message: t('renterRentalDetailPage.alerts.paymentUnderReview'), type: 'info' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        }
      }
    }
    ...
    if (token) {
      ...
      socketService.onProductUpdated((updatedProduct) => {
        if (rental && updatedProduct.id === rental.product_id) {
          ...
          setRealtimeNotification({ isVisible: true, message: t('renterRentalDetailPage.alerts.productUpdated'), type: 'info' });
          setTimeout(() => setRealtimeNotification(prev => ({ ...prev, isVisible: false })), 5000);
        }
      });
  ...
  const getReturnConditionText = (condition: RentalReturnConditionStatus): string => {
    switch (condition) {
      case RentalReturnConditionStatus.AS_RENTED: return t('renterRentalDetailPage.returnConditions.as_rented');
      case RentalReturnConditionStatus.MINOR_WEAR: return t('renterRentalDetailPage.returnConditions.minor_wear');
      case RentalReturnConditionStatus.DAMAGED: return t('renterRentalDetailPage.returnConditions.damaged');
      case RentalReturnConditionStatus.LOST: return t('renterRentalDetailPage.returnConditions.lost');
      default: return t('renterRentalDetailPage.returnConditions.not_specified');
    }
  };

  const getDeliveryStatusText = (status: string): string => {
    switch (status) {
      case 'pending': return t('renterRentalDetailPage.deliveryStatus.pending');
      case 'shipped': return t('renterRentalDetailPage.deliveryStatus.shipped');
      case 'delivered': return t('renterRentalDetailPage.deliveryStatus.delivered');
      case 'failed': return t('renterRentalDetailPage.deliveryStatus.failed');
      case 'returned': return t('renterRentalDetailPage.deliveryStatus.returned');
      default: return t('renterRentalDetailPage.deliveryStatus.unknown');
    }
  };
  ...
  const handleChatWithOwner = async () => {
    ...
    try {
      const messageText = t('renterRentalDetailPage.defaultChatMessage', { rentalId: rental.rental_uid, product: rental.product?.title || t('renterRentalDetailPage.product') });
      ...
      showError(err?.response?.data?.message || err?.message || t('renterRentalDetailPage.alerts.chatNotAvailable'));
    } finally {
  ...
  let statusBox = null;
  // ... statusBox logic remains the same ...
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 pt-16">
      ...
      <div className="container mx-auto p-4 md:p-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-6">
          <Link to={ROUTE_PATHS.MY_RENTALS_RENTER} className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 transition-colors mb-4"><FaArrowLeft className="h-4 w-4" />{t('renterRentalDetailPage.backToMyRentals')}</Link>
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1"><h1 className="text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">{rental.product?.title}</h1><p className="text-gray-600 text-lg">{t('renterRentalDetailPage.rentalDetailsSubtitle')}</p></div>
            <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${isSocketConnected ? 'bg-green-500' : 'bg-red-500'} animate-pulse`}></div>
                <span className="text-sm text-gray-600 flex items-center gap-1"><FaWifi className="h-4 w-4" />{isSocketConnected ? t('renterRentalDetailPage.connectionStatus.live') : t('renterRentalDetailPage.connectionStatus.offline')}</span>
                {isRealtimeConnected && (<span className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-700 border border-green-200"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>{t('realtimeActive')}</span>)}
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-gray-100 p-1 rounded-xl flex items-center gap-1 shadow-inner">
              <TabButton label={t('renterRentalDetailPage.tabs.overview', 'ภาพรวม')} icon={<FaInfoCircle />} isActive={activeTab === 'overview'} onClick={() => setActiveTab('overview')} />
              <TabButton label={t('renterRentalDetailPage.tabs.deliveryReturn', 'การจัดส่งและรับคืน')} icon={<FaTruck />} isActive={activeTab === 'delivery_return'} onClick={() => setActiveTab('delivery_return')} />
              <TabButton label={t('renterRentalDetailPage.tabs.payment', 'การชำระเงิน')} icon={<FaReceipt />} isActive={activeTab === 'payment'} onClick={() => setActiveTab('payment')} />
            </div>
                  ...
                  <div className="space-y-6">
                    {(productDetails?.primary_image?.image_url || rental.product?.primary_image?.image_url || (rental.product?.images && rental.product.images[0]?.image_url)) && (
                      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        <img src={ productDetails?.primary_image?.image_url || rental.product?.primary_image?.image_url || (rental.product?.images && rental.product.images[0]?.image_url) || '' } alt={productDetails?.title || rental.product?.title || t('renterRentalDetailPage.productImage')} className="w-full max-w-md max-h-72 h-auto object-cover rounded-xl shadow-lg mx-auto" />
                      </div>
                    )}
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                      <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-blue-100 rounded-xl"><FaInfoCircle className="h-6 w-6 text-blue-600" /></div><h3 className="text-2xl font-bold text-gray-800">{t('renterRentalDetailPage.productDetails')}</h3></div>
                      <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaBox className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.productName')}</span><p className="font-semibold">{productDetails?.title || rental.product?.title}</p></div></div>
                          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaShieldAlt className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.category')}</span><p className="font-semibold">{productDetails?.category?.name || rental.product?.category?.name}</p></div></div>
                          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaMoneyBillWave className="h-5 w-5 text-yellow-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.pricePerDay')}</span><p className="font-semibold">{formatCurrency((productDetails?.rental_price_per_day || rental.product?.rental_price_per_day) || 0)}</p></div></div>
                          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaShieldAlt className="h-5 w-5 text-purple-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.deposit')}</span><p className="font-semibold">{formatCurrency((productDetails?.security_deposit || rental.product?.security_deposit) || 0)}</p></div></div>
                        </div>
                ...
                {activeTab === 'delivery_return' && (
                  <div className="space-y-6">
                    ...
                    {rental.pickup_method === 'delivery' && rental.delivery_status && (
                      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-blue-100 rounded-xl"><FaTruck className="h-6 w-6 text-blue-600" /></div><h3 className="text-2xl font-bold text-gray-800">{t('renterRentalDetailPage.deliveryStatus.title')}</h3></div>
                        <div className="space-y-4">
                          <div className={`border-l-4 p-4 rounded ${getDeliveryStatusColor(rental.delivery_status)}`}><div className="flex items-center gap-3"><FaTruck className="h-5 w-5" /><div><span className="text-sm font-medium">{t('renterRentalDetailPage.deliveryStatus.status')}</span><p className="font-semibold">{getDeliveryStatusText(rental.delivery_status)}</p></div></div></div>
                          {rental.tracking_number && <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaInfoCircle className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.deliveryStatus.trackingNumber')}</span><p className="font-semibold">{rental.tracking_number}</p></div></div>}
                          {rental.carrier_code && <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaShieldAlt className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.deliveryStatus.carrier')}</span><p className="font-semibold">{rental.carrier_code}</p></div></div>}
                        </div>
                      </div>
                    )}
                    {rental.actual_return_time && (
                      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-green-100 rounded-xl"><FaCheckCircle className="h-6 w-6 text-green-600" /></div><h3 className="text-2xl font-bold text-green-800">{t('renterRentalDetailPage.returnInfo.title')}</h3></div>
                        <div className="space-y-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl"><FaCalendarAlt className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.returnInfo.returnedAt')}</span><p className="font-semibold">{new Date(rental.actual_return_time).toLocaleString()}</p></div></div>
                            {rental.return_condition_status && (<div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl"><FaShieldAlt className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.returnInfo.condition')}</span><p className="font-semibold">{getReturnConditionText(rental.return_condition_status)}</p></div></div>)}
                          </div>
                          {rental.notes_from_owner_on_return && (<div className="p-4 bg-blue-50 border border-blue-200 rounded-xl"><div className="flex items-center gap-2 mb-2"><FaUser className="h-5 w-5 text-blue-600" /><span className="font-semibold text-blue-800">{t('renterRentalDetailPage.returnInfo.notes')}</span></div><p className="text-blue-700">{rental.notes_from_owner_on_return}</p></div>)}
                          {rental.return_condition_image_urls && rental.return_condition_image_urls.length > 0 ? (<div><h4 className="font-semibold text-gray-800 mb-3">{t('renterRentalDetailPage.returnInfo.images')}</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{rental.return_condition_image_urls.map((imageUrl, index) => (<a key={`return-image-${index}`} href={imageUrl} target="_blank" rel="noopener noreferrer" className="group"><img src={imageUrl} alt={`${t('renterRentalDetailPage.returnEvidence')} ${index + 1}`} className="w-full h-24 object-cover rounded-lg border-2 border-gray-200 group-hover:border-blue-400 transition-colors" /></a>))}</div></div>) : (<div className="p-4 bg-gray-50 border border-gray-200 rounded-xl"><div className="flex items-center gap-2"><FaInfoCircle className="h-5 w-5 text-gray-500" /><span className="text-gray-600">{t('renterRentalDetailPage.returnInfo.noImages')}</span></div></div>)}
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            {typeof rental.late_fee_calculated === 'number' && rental.late_fee_calculated > 0 && (<div className="flex items-center gap-3 p-3 bg-red-50 rounded-xl"><FaExclamationTriangle className="h-5 w-5 text-red-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.lateFeeDeducted')}</span><p className="font-semibold text-red-600">{formatCurrency(rental.late_fee_calculated)}</p></div></div>)}
                            {typeof rental.security_deposit_refund_amount === 'number' && (<div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl"><FaMoneyBillWave className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.depositRefund')}</span><p className="font-semibold text-green-600">{formatCurrency(rental.security_deposit_refund_amount)}</p><p className="text-xs text-gray-500 mt-1">{t('renterRentalDetailPage.depositRefundNote')}</p>{rental.rental_status === 'late_return' && (<div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg"><div className="flex items-center gap-2 text-yellow-800"><FaExclamationTriangle className="h-4 w-4" /><span className="text-sm font-medium">คืนเลทแล้ว - กรุณาติดต่อเจ้าของผ่านแชทเพื่อรับเงินประกันคืน</span></div></div>)}</div></div>)}
                            <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl"><FaMoneyBillWave className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.subtotal')}</span><p className="font-semibold">{formatCurrency(rental.calculated_subtotal_rental_fee || 0)}</p></div></div>
                            <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl"><FaCheckCircle className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.totalPaid')}</span><p className="font-semibold">{formatCurrency((rental.final_amount_paid || rental.total_amount_due) || 0)}</p></div></div>
                        </div>
                    </div>
                    {Boolean(rental.payment_proof_url) && (
                      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                        <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-blue-100 rounded-xl"><FaCreditCard className="h-6 w-6 text-blue-600" /></div><h3 className="text-2xl font-bold text-gray-800">{t('renterRentalDetailPage.paymentSlip')}</h3></div>
                        <div className="space-y-4">
                          <div className="flex justify-center"><a href={rental.payment_proof_url!} target="_blank" rel="noopener noreferrer" className="group"><img src={rental.payment_proof_url!} alt={t('renterRentalDetailPage.paymentSlipAlt')} className="max-w-xs rounded-xl shadow-lg border-2 border-gray-200 group-hover:border-blue-400 transition-colors" /></a></div>
                          <div className="flex justify-center"><a href={rental.payment_proof_url!} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl font-semibold hover:from-blue-600 hover:to-indigo-600 transition-all duration-200 shadow-lg hover:shadow-xl"><FaDownload className="h-4 w-4" />{t('renterRentalDetailPage.viewDownloadSlip')}</a></div>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="space-y-6">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
              <div className="space-y-4 mb-8">
                ...
                {(rental.owner?.id || rental.owner_id) && (<motion.button whileHover={{ scale: contactingOwner ? 1 : 1.02 }} whileTap={{ scale: contactingOwner ? 1 : 0.98 }} onClick={handleChatWithOwner} disabled={contactingOwner} className={`w-full bg-gradient-to-r from-cyan-500 to-blue-500 text-white py-4 px-6 rounded-xl font-semibold hover:from-cyan-600 hover:to-blue-600 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-3 ${contactingOwner ? 'opacity-75 cursor-not-allowed' : ''}`}>{contactingOwner ? (<div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>) : (<FaComments className="h-5 w-5" />)}{contactingOwner ? t('renterRentalDetailPage.buttons.contactingOwner') : t('renterRentalDetailPage.buttons.chatWithOwner')}</motion.button>)}
              </div>
              <hr className="my-6 border-gray-200" />
              <div className="space-y-4"><h3 className="text-xl font-bold text-gray-800 mb-4">{t('renterRentalDetailPage.rentalDetailsTitle')}</h3><div className="space-y-3"><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaShieldAlt className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.rentalId')}</span><p className="font-semibold">{rental.rental_uid}</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaInfoCircle className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.statusLabel')}</span><p className="font-semibold">{t(`rentalStatus.${rental.rental_status}`, rental.rental_status ? rental.rental_status.replace(/_/g, ' ').toUpperCase() : 'UNKNOWN')}</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaCreditCard className="h-5 w-5 text-purple-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.paymentStatusLabel')}</span><p className="font-semibold">{t(`paymentStatus.${rental.payment_status}`, rental.payment_status ? rental.payment_status.replace(/_/g, ' ').toUpperCase() : 'UNKNOWN')}</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaCalendarAlt className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.rentalPeriod')}</span><p className="font-semibold">{new Date(rental.start_date).toLocaleDateString()} - {new Date(rental.end_date).toLocaleDateString()}</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaClock className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.actualPickup')}</span><p className="font-semibold">{rental.actual_pickup_time ? new Date(rental.actual_pickup_time).toLocaleString() : '-'}</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaHistory className="h-5 w-5 text-purple-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.actualReturn')}</span><p className="font-semibold">{rental.actual_return_time ? new Date(rental.actual_return_time).toLocaleString() : '-'}</p></div></div></div></div>
              <hr className="my-6 border-gray-200" />
              <div className="space-y-4"><h3 className="text-xl font-bold text-gray-800 mb-4">{t('renterRentalDetailPage.people')}</h3><div className="space-y-3"><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaUser className="h-5 w-5 text-blue-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.owner')}</span><p className="font-semibold">{rental.owner?.first_name} {rental.owner?.last_name} (@{rental.owner?.username})</p></div></div><div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl"><FaUser className="h-5 w-5 text-green-500" /><div><span className="text-sm text-gray-500">{t('renterRentalDetailPage.renter')}</span><p className="font-semibold">{rental.renter?.first_name} {rental.renter?.last_name} (@{rental.renter?.username})</p></div></div></div></div>
              {(() => { const canCancel = [RentalStatus.PENDING_OWNER_APPROVAL, RentalStatus.PENDING_PAYMENT].includes(rental.rental_status); return canCancel; })() && (<><hr className="my-6 border-gray-200" /><motion.button onClick={() => setShowCancelDialog(true)} className="w-full bg-gradient-to-r from-red-500 to-pink-500 text-white py-4 px-6 rounded-xl font-semibold hover:from-red-600 hover:to-pink-600 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-3"><FaBan className="h-5 w-5" />{t('renterRentalDetailPage.buttons.cancelRental')}</motion.button></>)}
            </div>
          </motion.div>
        </motion.div>
      </div>
