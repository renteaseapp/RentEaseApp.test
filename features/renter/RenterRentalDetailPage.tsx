...
FaExclamationTriangle, FaTimes, FaCreditCard, FaBox, FaStar, FaEye, FaDownload, FaUndo,
FaShieldAlt, FaMapMarkerAlt, FaHistory, FaInfoCircle, FaBan, FaTruck, FaWifi, FaComments, FaQuestionCircle
} from 'react-icons/fa';
...
onClick={onClick}
disabled={isDisabled}
className={`flex-1 flex flex-col items-center justify-center gap-2 p-3 sm:p-4 text-xs sm:text-sm font-semibold rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 ${ isActive ? 'bg-blue-600 text-white shadow-lg transform scale-105' : isCompleted ? 'bg-green-100 text-green-700 hover:bg-green-200' : isDisabled ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-600 hover:bg-blue-50 hover:text-blue-600 shadow-md' }`}
>
<div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-lg sm:text-xl ${ isActive ? 'bg-white text-blue-600' : isCompleted ? 'bg-green-600 text-white' : isDisabled ? 'bg-gray-300 text-gray-500' : 'bg-blue-100 text-blue-600' }`}>
{isCompleted ? <FaCheck /> : icon}
</div>
<div className="text-center">
<div className="font-bold text-xs sm:text-sm">ขั้นตอนที่ {stepNumber}</div>
    ...
    nextSteps: [
      {
        ...
        action: () => setActiveStep('step5_return')
      },
      {
...
{statusBox && <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="mb-6">{statusBox}</motion.div>}
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        {/* Step Tabs */}
        <div className="bg-gray-100 p-2 rounded-xl flex items-center gap-2 shadow-inner overflow-x-auto">
          <StepTab 
            stepNumber={1} 
            title="ส่งคำขอ" 
            icon={<FaFileAlt />} 
            isActive={activeStep === 'step1_request'} 
            isCompleted={['pending_payment', 'confirmed', 'active', 'completed'].includes(rental.rental_status as RentalStatus)}
            isDisabled={false}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeStep} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                    ...
                    <div className="p-4 mt-4 bg-green-50 border border-green-200 rounded-xl">
                      ...
                      <div className="space-y-2 text-sm mb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span>{productDetails?.address_details || rental.product?.address_details || 'ไม่ได้ระบุที่อยู่'}</span>
                        </div>
                        {/* แสดงข้อมูลเพิ่มเติมจากข้อมูลเจ้าของหากมี */}
                        {rental.owner && (
                          <div className="flex items-center gap-2">
                            <FaUser className="h-4 w-4 text-gray-500" />
                            <span className="text-gray-600">ติดต่อ: {rental.owner.first_name} {rental.owner.last_name}</span>
                          </div>
                        )}
                      </div>
                      
                      {/* แสดงแผนที่เฉพาะเมื่อมีพิกัด */}
                      {(productDetails?.latitude || rental.product?.latitude) && (productDetails?.longitude || rental.product?.longitude) ? (
                        <>
                          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden mb-3">
                            <OpenStreetMapPicker
                        ...
                        /* แสดงข้อความแจ้งเตือนเมื่อไม่มีพิกัด */
                        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                          <div className="flex items-center gap-2">
                            <FaInfoCircle className="h-4 w-4 text-yellow-600" />
                            <span className="text-sm text-yellow-800">ไม่มีข้อมูลพิกัดสถานที่ กรุณาติดต่อเจ้าของสินค้าเพื่อขอรายละเอียดเพิ่มเติม</span>
                          </div>
                          <div className="mt-2 flex justify-end">
                            <button
                              onClick={handleChatWithOwner}
                              disabled={contactingOwner}
                              className="px-3 py-1 bg-yellow-600 text-white text-sm rounded-lg hover:bg-yellow-700 transition-colors flex items-center gap-2 disabled:opacity-50"
                            >
                              <FaComments className="w-3 h-3" />
                              {contactingOwner ? "กำลังติดต่อ..." : "ติดต่อเจ้าของ"}
                            </button>
                          </div>
                        </div>
                      )}
            ...
            {activeStep === 'step4_usage' && (
              <div className="space-y-6">
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                  <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-purple-100 rounded-xl"><FaEye className="h-6 w-6 text-purple-600" /></div><h3 className="text-2xl font-bold text-gray-800">{"การใช้งานสินค้า"}</h3></div>
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                      <div className="flex items-center gap-2 mb-2"><FaInfoCircle className="h-5 w-5 text-blue-600" /><span className="font-semibold text-blue-800">{"ข้อมูลการใช้งาน"}</span></div>
                      <p className="text-blue-700">{"คุณสามารถใช้งานสินค้านี้ได้ตามระยะเวลาที่เช่า กรุณาดูแลรักษาสินค้าให้อยู่ในสภาพดี"}</p>
                    </div>
                    {rental.actual_pickup_time && (
                      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                        ...
                        <div>
                          ...
                          <p className="font-semibold">{new Date(rental.actual_pickup_time).toLocaleString('th-TH')}</p>
                        </div>
                      </div>
                    )}
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      ...
                      <div>
                        <span className="text-sm text-gray-500">{"วันที่เริ่มเช่า"}</span>
                        <p className="font-semibold">{new Date(rental.start_date).toLocaleDateString('th-TH')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      <FaCalendarAlt className="h-5 w-5 text-orange-500" />
                      <div>
                        <span className="text-sm text-gray-500">{"วันที่สิ้นสุดการเช่า"}</span>
                        <p className="font-semibold">{new Date(rental.end_date).toLocaleDateString('th-TH')}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {activeStep === 'step5_return' && (
              <div className="space-y-6">
                <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
                  <div className="flex items-center gap-3 mb-6"><div className="p-2 bg-orange-100 rounded-xl"><FaUndo className="h-6 w-6 text-orange-600" /></div><h3 className="text-2xl font-bold text-gray-800">{"การคืนสินค้า"}</h3></div>
                  <div className="space-y-4">
                    <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl">
                      <div className="flex items-center gap-2 mb-2"><FaExclamationTriangle className="h-5 w-5 text-orange-600" /><span className="font-semibold text-orange-800">{"ข้อมูลการคืนสินค้า"}</span></div>
                      <p className="text-orange-700">{"กรุณาคืนสินค้าในสภาพดีตามที่ได้รับมา และตรงตามเวลาที่กำหนด"}</p>
                    </div>
                    {rental.actual_return_time && (
                      <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                        ...
                        <div>
                          ...
                          <p className="font-semibold">{new Date(rental.actual_return_time).toLocaleString('th-TH')}</p>
                        </div>
                      </div>
                    )}
                        ...
                        {rental.rental_status === RentalStatus.LATE_RETURN && (
                          <div className="mt-2 flex items-center gap-2">
                            ...
                            <p className="text-sm text-red-600 font-medium">
                              {"⚠️ เกินกำหนดคืนแล้ว - อาจมีค่าปรับเพิ่มเติม"}
                            </p>
                          </div>
                        )}
                        {rental.rental_status === RentalStatus.ACTIVE && new Date() > new Date(rental.end_date) && (
                          <div className="mt-2 flex items-center gap-2">
                            <FaExclamationTriangle className="h-4 w-4 text-red-500" />
                            <p className="text-sm text-red-600 font-medium">
                              {"⚠️ เกินกำหนดคืนแล้ว - กรุณาแจ้งคืนสินค้าโดยเร็ว"}
                            </p>
                          </div>
                        )}
                        {rental.rental_status === RentalStatus.ACTIVE && new Date() <= new Date(rental.end_date) && (
                          <div className="mt-2">
                            <p className="text-sm text-blue-600">
                              {"💡 กรุณาเตรียมคืนสินค้าภายในวันที่กำหนด"}
                            </p>
                          </div>
                        )}
                      </div>
      ...
      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="space-y-6">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
          ...
          <div className="space-y-4"><h3 className="text-xl font-bold text-gray-800 mb-4">{"รายละเอียดการเช่า"}</h3><div className="space-y-3">
              ...
              {/* วันที่ต้องคืนสินค้า - ปรับปรุงให้เด่นชัดขึ้น */}
