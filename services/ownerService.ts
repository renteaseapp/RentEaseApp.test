...
export const getPayoutMethodsByOwnerId = async (ownerId: number): Promise<PayoutMethod[]> => {
    ...
    const response = await fetch(`http://localhost:3001/api/owners/${ownerId}/payout-methods`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
...
export const updateRentalDeliveryStatus = async (
  ...
  const response = await fetch(`${API_BASE_URL}/owner/rentals/${rentalId}/delivery-status`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
