...
export const adminGetComplaints = async (params: { page?: number; limit?: number } = {}): Promise<PaginatedResponse<Complaint>> => {
  const res = await api.get('/admin/complaints', { params });
  return res.data;
};
export const adminGetComplaintById = async (id: number): Promise<Complaint> => {
...
// --- 7. Static Pages ---
export const adminGetStaticPages = async (): Promise<{ data: StaticPageContent[] }> => {
  const res = await api.get('/admin/static-pages');
  return res.data;
};
export const adminUpdateStaticPage = async (slug: string, payload: Partial<StaticPageContent>): Promise<StaticPageContent> => {
  const res = await api.put(`/admin/static-pages/${slug}`, payload);
  return res.data;
};
